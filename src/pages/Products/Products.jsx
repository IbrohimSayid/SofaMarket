...
function MediaCard({ product }) {
  const { image, title, category, company, price } = product.attributes;

  return (
    <div className="container-top container mx-auto px-4">
      <div className="border border-gray-300 rounded-lg shadow-lg overflow-hidden bg-white">
        ...
        <div className="p-4">
          <h5 className="text-xl font-bold mb-2 text-gray-800">{title}</h5>
          <p className="text-sm text-gray-500 mb-1">{category}</p>
          <p className="text-sm text-gray-500">{company}</p>
          <p className="text-lg text-blue-600 mt-4 font-semibold">${price}</p>
        </div>
      </div>
    </div>
  );
...
function Form({ filter }) {
  ...
  function handleSearch() {
    let copiedProducts = [...filter.products];
